    ...
    internal static class Program
    {
        ...
        private static async Task AddDefaultRequestHeadersAsync()
        {
            using (var unit = IoC.Resolve<IUnitOfWork>())
            {
                ...
                var list = new List<RequestHeader> { new RequestHeader { Key = "user-agent", Value = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.181 Safari/537.36" }, new RequestHeader { Key = "accept-encoding", Value = "gzip, deflate" }, new RequestHeader { Key = "accept-language", Value = "en-US,en;q=0.8,de;q=0.6" }, new RequestHeader { Key = "upgrade-insecure-requests", Value = "1" } };
